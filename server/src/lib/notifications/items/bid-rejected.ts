...
class BidRejectedNotification implements FCMNotificationItem {
  send = async (bid: Bid) => {
    try {
      ...
      if (
        ownerOfAuction.allowedNotifications.BID_REJECTED_ON_AUCTION === false
      ) {
        return
      }
      ...
      let notification = new Notification({
        ...
        type: NotificationTypes.BID_REJECTED_ON_AUCTION,
      ...
      console.error('Coult not send bid rejected notification', error)
    }
  }
}

const notificationInstance = new BidRejectedNotification()
export { notificationInstance as BidRejectedNotification }
