...
export async function down({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    await queryInterface.removeColumn(DATABASE_MODELS.AUCTIONS, 'startAt', { transaction })
    await queryInterface.removeColumn(DATABASE_MODELS.AUCTIONS, 'startedAt', { transaction })
