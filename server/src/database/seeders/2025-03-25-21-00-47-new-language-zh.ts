...
export async function up({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    ...
    for (const category of allCategories) {
      const name = await getTranslation('en', 'zh-cn', category.name.en)
      category.name['zh'] = name

      if (category.details) {
        const details = await getTranslation('en', 'zh-cn', category.details.en)
        if (details) {
          category.details['zh'] = details
        }
      }
    ...
    for (const notificationContent of notificationsContent) {
      const title = await getTranslation('en', 'zh-cn', notificationContent.title.en)
      const description = await getTranslation('en', 'zh-cn', notificationContent.description.en)
      notificationContent.title['zh'] = title
      notificationContent.description['zh'] = description
    ...
    for (const currency of allCurrencies) {
      const name = await getTranslation('en', 'zh-cn', currency.name.en)
      currency.name['zh'] = name
