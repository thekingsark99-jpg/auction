...
export async function up({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    ...
    for (const category of allCategories) {
      const name = await getTranslation('en', 'tr', category.name.en)
      category.name['tr'] = name

      if (category.details) {
        const details = await getTranslation('en', 'tr', category.details.en)
        if (details) {
          category.details['tr'] = details
        }
      }
    ...
    for (const notificationContent of notificationsContent) {
      const title = await getTranslation('en', 'tr', notificationContent.title.en)
      const description = await getTranslation('en', 'tr', notificationContent.description.en)
      notificationContent.title['tr'] = title
      notificationContent.description['tr'] = description
    ...
    for (const currency of allCurrencies) {
      const name = await getTranslation('en', 'tr', currency.name.en)
      currency.name['tr'] = name
