...
export async function up({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    ...
    for (const category of allCategories) {
      const name = await getTranslation('en', 'pl', category.name.en)
      category.name['pl'] = name

      if (category.details) {
        const details = await getTranslation('en', 'pl', category.details.en)
        if (details) {
          category.details['pl'] = details
        }
      }
    ...
    for (const notificationContent of notificationsContent) {
      const title = await getTranslation('en', 'pl', notificationContent.title.en)
      const description = await getTranslation('en', 'pl', notificationContent.description.en)
      notificationContent.title['pl'] = title
      notificationContent.description['pl'] = description
    ...
    for (const currency of allCurrencies) {
      const name = await getTranslation('en', 'pl', currency.name.en)
      currency.name['pl'] = name
