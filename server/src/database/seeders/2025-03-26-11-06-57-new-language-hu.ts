...
export async function up({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    ...
    for (const category of allCategories) {
      const name = await getTranslation('en', 'hu', category.name.en)
      category.name['hu'] = name

      if (category.details) {
        const details = await getTranslation('en', 'hu', category.details.en)
        if (details) {
          category.details['hu'] = details
        }
      }
    ...
    for (const notificationContent of notificationsContent) {
      const title = await getTranslation('en', 'hu', notificationContent.title.en)
      const description = await getTranslation('en', 'hu', notificationContent.description.en)
      notificationContent.title['hu'] = title
      notificationContent.description['hu'] = description
    ...
    for (const currency of allCurrencies) {
      const name = await getTranslation('en', 'hu', currency.name.en)
      currency.name['hu'] = name
