...
export async function up({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    ...
    for (const category of allCategories) {
      const name = await getTranslation('en', 'hi', category.name.en)
      category.name['hi'] = name

      if (category.details) {
        const details = await getTranslation('en', 'hi', category.details.en)
        if (details) {
          category.details['hi'] = details
        }
      }
    ...
    for (const notificationContent of notificationsContent) {
      const title = await getTranslation('en', 'hi', notificationContent.title.en)
      const description = await getTranslation('en', 'hi', notificationContent.description.en)
      notificationContent.title['hi'] = title
      notificationContent.description['hi'] = description
    ...
    for (const currency of allCurrencies) {
      const name = await getTranslation('en', 'hi', currency.name.en)
      currency.name['hi'] = name
