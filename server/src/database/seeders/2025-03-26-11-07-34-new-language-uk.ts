...
export async function up({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    ...
    for (const category of allCategories) {
      const name = await getTranslation('en', 'uk', category.name.en)
      category.name['uk'] = name

      if (category.details) {
        const details = await getTranslation('en', 'uk', category.details.en)
        if (details) {
          category.details['uk'] = details
        }
      }
    ...
    for (const notificationContent of notificationsContent) {
      const title = await getTranslation('en', 'uk', notificationContent.title.en)
      const description = await getTranslation('en', 'uk', notificationContent.description.en)
      notificationContent.title['uk'] = title
      notificationContent.description['uk'] = description
    ...
    for (const currency of allCurrencies) {
      const name = await getTranslation('en', 'uk', currency.name.en)
      currency.name['uk'] = name
