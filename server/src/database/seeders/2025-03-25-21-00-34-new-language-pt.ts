...
export async function up({ context: queryInterface }: { context: sequelize.QueryInterface }) {
  ...
  try {
    ...
    for (const category of allCategories) {
      const name = await getTranslation('en', 'pt', category.name.en)
      category.name['pt'] = name

      if (category.details) {
        const details = await getTranslation('en', 'pt', category.details.en)
        if (details) {
          category.details['pt'] = details
        }
      }
    ...
    for (const notificationContent of notificationsContent) {
      const title = await getTranslation('en', 'pt', notificationContent.title.en)
      const description = await getTranslation('en', 'pt', notificationContent.description.en)
      notificationContent.title['pt'] = title
      notificationContent.description['pt'] = description
    ...
    for (const currency of allCurrencies) {
      const name = await getTranslation('en', 'pt', currency.name.en)
      currency.name['pt'] = name
